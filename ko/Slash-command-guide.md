...
## Incoming Webhook 으로 메시지를 전송하는 방법(responseUrl)

커맨드가 특성에 따라 임의의 시간에 메시지를 보내야 하는 경우가 있습니다.(예시: 매일 특정 시간에 오늘의 일정을 알려주는 커맨드) 이 경우에는 responseUrl을 활용해 메시지를 전송합니다.
...
당장 메시지를 반환할 필요가 없는 커맨드일 경우에도 사용자의 행동 즉시 메시지를 반환하도록 처리해 주세요. ("등록이 완료되었습니다.", "매일 아침 08:00에 알려드리겠습니다." 등)
responseUrl로만 메시지를 보낼 경우 메신저 서버에서는 정상적인 응답으로 간주하지 않아 time out이 발생하게 됩니다.
